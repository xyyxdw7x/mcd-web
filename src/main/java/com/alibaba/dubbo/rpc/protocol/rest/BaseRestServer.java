...
public abstract class BaseRestServer implements RestServer {
	...
	public void start(URL url) {
		...
		// users can override this mapper, but we just rely on the current
		// priority strategy of resteasy
	...
	public void deploy(Class<?> resourceDef, Object resourceInstance,
		...
		if (StringUtils.isEmpty(contextPath)) {
			getDeployment().getRegistry().addResourceFactory(
					new DubboResourceFactory(resourceInstance, resourceDef));
		} else {
			getDeployment().getRegistry().addResourceFactory(
					new DubboResourceFactory(resourceInstance, resourceDef),
					contextPath);
