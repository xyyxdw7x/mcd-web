package com.asiainfo.biapp.framework.web.mvc;
...
/**
 * 更强大的RequestMappingHandlerMapping注解
 * spring mvc默认必须在方法名上添加路径才可以正确映射 例如@RequestMapping("/findBooks")
 * 本类实现只需要@RequestMapping 路径为默认的方法名 而且支持自定义后缀名
 * @author hjn
 *
 */
public class PowerfulRequestMappingHandlerMapping extends RequestMappingInfoHandlerMapping
	 ...
	 */
	protected RequestMappingInfo createRequestMappingInfo(
			RequestMapping requestMapping, RequestCondition<?> customCondition,Method method) {
        String[] paths=new String[]{"/"+method.getName()+"."+getExtension()};
		return RequestMappingInfo
