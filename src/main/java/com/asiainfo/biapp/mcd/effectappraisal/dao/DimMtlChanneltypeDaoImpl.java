...
import com.asiainfo.biapp.mcd.model.DimMtlChannel;
 ...
 *
 */
@Repository(value="dimMtlChanneltypeDao")
public class DimMtlChanneltypeDaoImpl extends JdbcDaoBase implements IDimMtlChanneltypeDao {
	...
	public List<DimMtlChanneltype> getMtlChanneltypeByCondition(String isDoubleSelect){
		...
		final String strSql = sql;
		return this.getJdbcTemplate().queryForList(sql, DimMtlChanneltype.class);
	}
	...
	public List<DimMtlChanneltype> getChannelMsg(String isDoubleSelect) {
		...
		try {
			...
			list = jt.queryForList(sbuffer.toString());
			for (Map map : list) {
	...
	public List<DimPlanSrvType> getGradeList() throws MpmException {
		...
		try {
			...
			log.debug("初始化粒度，sql="+sbuffer.toString());
