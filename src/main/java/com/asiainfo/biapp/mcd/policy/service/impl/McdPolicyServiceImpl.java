...
public class McdPolicyServiceImpl implements IMcdPolicyService{
	...
	public List<Map<String, Object>> getPolicyByCondition(String typeId, String statusId, String keyWords,
		...
		//主sql
		Map<String,Object> sqlClause = this.getPolicysByConditionSql(typeId, statusId, keyWords, keyWords,pager);
	...
	public JSONObject queryDetail(String planId) throws Exception {
		...
		//查询状态
		List<McdDimPlanOnlineStatus> planStatus = mcdPolicyDao.initDimPolicyStatus();
		//详情内容总体查询
	...
	private Boolean updateDealCode(String planId, String dealCode_10086, String dealCode_1008611, String urlForAndroid,
		...
		if (bfResult == null || bfResult.size() == 0) {
			...
			param1.add(manager);
			return mcdPolicyDao.updatePolicy(upSql1.toString(), param1);
		} else {
			StringBuffer upSql1 = new StringBuffer("");
			List<Object> param1 = new ArrayList<Object>();
	...
	private Boolean updateExecCon(String planId, String channelId) {
		...
		mcdPolicyDao.updatePolicy(deleteChannel, deleteParams);
		...
		for (int i = 0; i < channel.length; i++) {
			...
			mcdPolicyDao.updatePolicy(sql3.toString(), upResults);
	...
	private Boolean updateScoreAward(String planId, String cityIds, String scores, String awards) {
		...
		mcdPolicyDao.updatePolicy(scoresDelete.toString(), scoresParam);
		...
		mcdPolicyDao.updatePolicy(awardsDelete.toString(), awardsParam);
		...
		for (int i = 0; i < cityId.length; i++) {
			...
			mcdPolicyDao.updatePolicy(scoresInTo.toString(), scoresInToParam);
		...
		for (int i = 0; i < cityId.length; i++) {
			...
			mcdPolicyDao.updatePolicy(awardsInTo.toString(), awardsInToParam);
