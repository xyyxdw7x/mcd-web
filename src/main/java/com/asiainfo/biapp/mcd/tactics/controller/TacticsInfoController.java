 ...
 *
 */
@RequestMapping("/action/tactics/tacticsInfo")
public class TacticsInfoController extends BaseMultiActionController {
	private static Logger log = LogManager.getLogger(TacticsInfoController.class);
	...
	@RequestMapping
	@ResponseBody
	public Map<String, Object> viewPolicyDetail(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		try {
			...
			if (planid != null && !"".equals(planid)) {
				...
				stcPlanJSON.put("stcPlanStartdate",
						stcPlan != null ? DateFormatUtils.format(stcPlan.getPlanStartdate(), "yyyy/MM/dd") : ""); //“有效期”开始时间
				stcPlanJSON.put("stcPlanEnddate",
						stcPlan != null ? DateFormatUtils.format(stcPlan.getPlanEnddate(), "yyyy/MM/dd") : ""); //“有效期”结束时间
	...
	@RequestMapping
	@ResponseBody
	public Map<String, Object> getTargetCustomerbase(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		Map<String, Object> returnMap = new HashMap<String, Object>();
		//投放的渠道
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				create_user_id = tmap.get("create_user_id") == null ? null : strNullToStr((String) tmap
						.get("create_user_id"));
				custom_num = tmap.get("custom_num") == null ? null : (BigDecimal) tmap.get("custom_num");
				custom_source_id = tmap.get("custom_source_id") == null ? null : strNullToStr((String) tmap
						.get("custom_source_id"));
				rule_desc = tmap.get("rule_desc") == null ? null : strNullToStr((String) tmap.get("rule_desc"));
				custgroup_number = tmap.get("targer_user_nums") == null ? null : (BigDecimal) tmap
						.get("targer_user_nums");
			}
			...
			if (update_cycle == null) {
				...
				customDataJSON.put("update_cycle",update_cycle == null ? "" : CustUpdateCycle.getName((short) Integer.parseInt(String
								.valueOf(update_cycle))));
			}
			...
			for (int i = 0; i < ruleList.size(); i++) {
				...
				if (elementValue.indexOf(",") != -1) {
					...
					if ("1".equals(elementValue)) {
						...
						resultStr += " " + columnCnName + ":[" + elementValue.split("-")[0] + ","
	...
	@RequestMapping
	@ResponseBody
	public Map<String, Object> getDeliveryChannel(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		McdCampDef segInfoBean = new McdCampDef();
		Map<String, Object> returnMap = new HashMap<String, Object>();
		//投放的渠道
		try {
			...
			//boss运营位短信模板
			List<ChannelBossSmsTemplate> bossSmsTemplatelist = channelBossSmsTemplateService
					.initMtlChannelBossSmsTemplate();
			for (int i = 0; i < list.size(); i++) {
				Map<String, Object> tmap = list.get(i);
		...
		try {
			...
			if (mtlCampSeginfo.getApproveFlowId() != null) {
				...
				for (int i = 0; i < elementList.size(); i++) {
					...
					String approve_result = element.element("APPROVE_RESULT") != null ? element.element(
							"APPROVE_RESULT").getText() : "";
					String approve_view = element.element("APPROVE_VIEW") != null ? element.element("APPROVE_VIEW")
							.getText() : "";
					String approvaler_name = element.element("APPROVALER_NAME") != null ? element.element(
							"APPROVALER_NAME").getText() : "";
					String approve_date = element.element("APPROVE_DATE") != null ? element.element("APPROVE_DATE")
							.getText() : "";
					String if_current_node = element.element("IF_CURRENT_NODE") != null ? element.element(
							"IF_CURRENT_NODE").getText() : "";
	...
	@RequestMapping
	@ResponseBody
	public Map<String, Object> tokenVerification(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String campsegId = request.getParameter("campsegId");
	...
	@RequestMapping
	@ResponseBody
	public Map<String, Object> getCampChannels(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
	...
	@RequestMapping
	@ResponseBody
	public Map<String, Object> getCampChannelDetail(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		...
		try {
			List<Map<String, Object>> list = mpmCampSegInfoService.getCampChannelDetail(campsegId, channelId,
					startDate, endDate);
			dg.setDatas(list);
			if ("901".equals(channelId)) {
				dg.setHeaderTexts(new String[] { "执行时间", "计划群发客户量", "已发送量/过滤数量", "当日成功办理量", "累计接触客户量", "累计成功办理量" });
				dg.setDataFields(new String[] { "DATA_DATE", "TARGET_NUMS", "EXCUTE_SURPLUS", "SAILSUCC_NUMS",
						"CUMULAT_SAIL_NUMS", "CUMULAT_SAILSUCC_NUMS" });
			} else if ("908".equals(channelId)) {//外呼
				dg.setHeaderTexts(new String[] { "执行时间", "计划外呼量", "已外呼数量", "当日成功办理量", "累计接触客户量", "累计成功办理量" });
				dg.setDataFields(new String[] { "DATA_DATE", "TARGET_NUMS", "SEND_NUMS", "SAILSUCC_NUMS",
						"CUMULAT_SAIL_NUMS", "CUMULAT_SAILSUCC_NUMS" });
			} else if ("910".equals(channelId)) {//BOSS运营位
				dg.setHeaderTexts(new String[] { "执行时间", "当日接触客户量", "当日成功办理量", "累计接触客户量", "累计成功办理量" });
				dg.setDataFields(new String[] { "DATA_DATE", "SAIL_NUMS", "SAILSUCC_NUMS", "CUMULAT_SAIL_NUMS",
						"CUMULAT_SAILSUCC_NUMS" });
			} else {
				dg.setHeaderTexts(new String[] { "执行时间", "当日展示政策次数", "累计展示政策次数", "当日点击政策次数", "累计点击政策次数", "当日成功办理量",
						"累计成功办理量" });
				dg.setDataFields(new String[] { "DATA_DATE", "EXPOSURE_NUMS", "CUMULAT_EXPOSURE_NUMS", "CK_NUMS",
						"CUMULAT_CK_NUMS", "SAILSUCC_NUMS", "CUMULAT_SAILSUCC_NUMS" });
			}
			dg.setDatas(list);
			returnMap.put("status", "200");
	...
	@RequestMapping
	@ResponseBody
	public Map<String, Object> getCampsChannelSituation(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		...
		DataGridData dg = new DataGridData();
		try {
			List<Map<String, Object>> list = mpmCampSegInfoService.getCampsChannelSituation(campsegIds, channelId,
					statDate);
			dg.setDatas(list);
			if ("901".equals(channelId)) {
				dg.setHeaderTexts(new String[] { "政策名称", "计划群发客户量", "已发送量/过滤数量", "当日成功办理量", "累计接触客户量", "累计成功办理量" });
				dg.setDataFields(new String[] { "PLAN_NAME", "TARGET_NUMS", "EXCUTE_SURPLUS", "SAILSUCC_NUMS",
						"CUMULAT_SAIL_NUMS", "CUMULAT_SAILSUCC_NUMS" });
			} else if ("908".equals(channelId)) {//外呼
				dg.setHeaderTexts(new String[] { "政策名称", "计划外呼量", "已外呼数量", "当日成功办理量", "累计接触客户量", "累计成功办理量" });
				dg.setDataFields(new String[] { "PLAN_NAME", "TARGET_NUMS", "SEND_NUMS", "SAILSUCC_NUMS",
						"CUMULAT_SAIL_NUMS", "CUMULAT_SAILSUCC_NUMS" });
			} else if ("910".equals(channelId)) {//BOSS运营位
				dg.setHeaderTexts(new String[] { "政策名称", "当日接触客户量", "当日成功办理量", "累计接触客户量", "累计成功办理量" });
				dg.setDataFields(new String[] { "PLAN_NAME", "SAIL_NUMS", "SAILSUCC_NUMS", "CUMULAT_SAIL_NUMS",
						"CUMULAT_SAILSUCC_NUMS" });
			} else {
				dg.setHeaderTexts(new String[] { "政策名称", "当日展示政策次数", "累计展示政策次数", "当日点击政策次数", "累计点击政策次数", "当日成功办理量",
						"累计成功办理量" });
				dg.setDataFields(new String[] { "PLAN_NAME", "EXPOSURE_NUMS", "CUMULAT_EXPOSURE_NUMS", "CK_NUMS",
						"CUMULAT_CK_NUMS", "SAILSUCC_NUMS", "CUMULAT_SAILSUCC_NUMS" });
			}
			dg.setDatas(list);
			returnMap.put("status", "200");
