define(["backbone"],function(require, exports,module) {      
	var generalModel = Backbone.Model.extend({
		urlRoot : _ctx+"/action/mpm/bullManage",
		defaults : {_ctx : _ctx}
	}); 
	module.exports={
		 init:function(){
		 ...
		 loadBulkList:function(){
			 var tacticsStateTabView = Backbone.View.extend({
									...
									buttons: [
										{
											...
											click: function() {
												...
												$.ajax({
													   type: "POST",
													   url: _ctx+"/action/mpm/bullManage/batchUpdatePauseComment.do",
													   data: {campIds:campIds,pauseComment:PauseComment},
									...
									buttons: [
										{
											...
											click: function() {
												...
												$.ajax({
													   type: "POST",
													   url: _ctx+"/action/mpm/bullManage/batchUpdatePauseComment.do",
													   data: {campIds:ids,pauseComment:PauseComment},
													   success: function(msg){
													     $(".more_tips").attr("pauseComment",PauseComment);
													     _this.taskStartStop({
																cmd:"stopTask.do",
					...
					loadList:function(deptId){
						var _that = this;
						var bulkModel = new generalModel({id:"viewBullAjax.do"});
						bulkModel.fetch({
							type : "post",
							...
							success:function(model){
								...
								$("#sortListItem").html(html).sortable({
									...
									stop:function(event, ui){
										var bulkSortModel = new generalModel({id:"setCampPri.do"});
...
//													alert("更新优先级成功！");
					...
					loadDepteList:function(){
						var bulkDepteModel = new generalModel({id:"allDeptes.do"});
						var _that = this;
					...
					taskStartStop:function(ajaxData){
						var idStr = ajaxData.cmd;
						var taskModel = new generalModel({id:idStr});
						var _that = this;
						taskModel.fetch({
							type : "post",
							contentType: "application/x-www-form-urlencoded; charset=utf-8",
					...
					bulkSendType:function(sendType){
						var sendTypeModel = new generalModel({id:"setSendType.do"});
						sendTypeModel.fetch({
							type : "post",
							contentType: "application/x-www-form-urlencoded; charset=utf-8",
							dataType:'json',
