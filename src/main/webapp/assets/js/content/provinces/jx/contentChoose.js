...
chooseView.addContentTypeChooseView = function(){
    ...
    //获取json数据
    $.ajax({
        url:contextPath+"/action/content/contentManage/queryContentTypes.do",
        type:"GET",
        async:false,
        success:function(data) {
            if(!data){return ;}
             contentStatus="";
             contentStatus = data.contentStatus;//状态

             /**状态视图*/
             var statusView = "<span statusId='' class='active'>全部</span>";//状态视图
             for(var i=0;i<contentStatus.length;i++){
            	 var statusId = contentStatus[i].statusId;
            	 var statusName = contentStatus[i].statusName;
