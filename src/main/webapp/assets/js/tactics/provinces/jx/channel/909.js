...
channelInfo909.initValue909 = function(){
	...
	if(tacticsInfo.plan.planComment!=null&&tacticsInfo.plan.planComment!=undefined){
		$("#channelId_"+channelInfo909.baseInfo.channelId+"_contentWords").val(tacticsInfo.plan.planComment);
	}
	
	//如果营销用语内容存在则需要更新营销用语、营销用语的可输入长度
	if(channelInfo909.baseInfo.hasOwnProperty("execContent")){
		//更新营销用语
...
channelInfo909.clickCommitButtonEventHandler909=function(){
	$("#commitButton_channelId_"+channelInfo909.baseInfo.channelId).click(function(){
		...
		if($("#commitButton_channelId_"+channelInfo909.baseInfo.channelId).hasClass("disable-href")){
			return ;
		}
...
channelInfo909.textAreaInputNumTip909=function(){
	...
	var $commitButton=$("#commitButton_channelId_"+channelInfo909.baseInfo.channelId);
