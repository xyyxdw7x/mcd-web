...
channelInfo910.loadSMtempaltes910=function(){
	...
	var url = contextPath;
	var templateLi = 
		'<li  data-toggle="tooltip" data-placement="bottom" titcontent="{Tooltip}" title="{Tooltip2}" smtemp="{smtemp}" id="{liId}">\n'+
			...
			if (retData) {
				...
				for(var index in retData){
					...
					for(var index2 in templates){
						...
						templateHtml = templateHtml.replace("{Tooltip2}", templates[index2].templateName+"——"+templates[index2].templateContent);
