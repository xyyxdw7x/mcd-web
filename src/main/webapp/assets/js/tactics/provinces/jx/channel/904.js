...
channelInfo904.initValue904 = function(){
	...
	if(tacticsInfo.plan.planComment!=null&&tacticsInfo.plan.planComment!=undefined){
		$("#channelId_"+channelInfo904.baseInfo.channelId+"_contentWords").val(tacticsInfo.plan.planComment);
	}
	
	//如果营销用语内容存在则需要更新营销用语、营销用语的可输入长度
	if(channelInfo904.baseInfo.hasOwnProperty("execContent")){
		//更新营销用语
...
channelInfo904.clickCommitButtonEventHandler904=function(){
	$("#commitButton_channelId_"+channelInfo904.baseInfo.channelId).click(function(){
		...
		if($("#commitButton_channelId_"+channelInfo904.baseInfo.channelId).hasClass("disable-href")){
			return ;
		}
...
channelInfo904.textAreaInputNumTip904=function(){
	...
	var $commitButton=$("#commitButton_channelId_"+channelInfo904.baseInfo.channelId);
