define(["backbone","jqueryUI","bootstrap"],function(require, exports,module) {     
	module.exports={
		init:function(){
	...
	if($(obj).find('span').hasClass('hidden')){
		...
		if(adivJson){
			for(var i = 0; i < adivJson.length; i++){
				_html += '<td class="fleft content-type-box" onclick="clickAdiv(this)" adiv_id="'+adivJson[i].adiv_id+'">'+adivJson[i].adiv_name+'</td>'
			}
...
function add_on_top(ele) {
	var _url = _ctx + '/mpm/priorityAction';
...
//				alert('置顶成功!');
	...
	setTimeout("on_top_manual("+chanID+")",2000);  //2000毫秒，既是2秒~
}
...
function load_channel(_cmd, _lct) {
	$.ajax({
		url : _ctx + '/mpm/imcdChannelExecuteAction?cmd=' + _cmd,
		type : "POST",
		data : {},
		success : function(result) {
			...
			$.ajax({
				url : _ctx + '/mpm/imcdChannelExecuteAction?cmd=initAdivInfo',
				type : "POST",
				async:false,
				data : {},
				success : function(result) {
					...
					for(var i = 0; i < adiv_data.length; i++){
						...
						var channel = $('#'+_lct+' .J_campType[cnid='+adiv.CHANNEL_ID+']');
						var channel_adiv_count = channel.attr('adiv-count');
						channel_adiv_count++;
						channel.attr('adiv-count',channel_adiv_count);

						if(channel.attr('adiv_info')!='' && channel.attr('adiv_info')!=undefined){
							var adiv_info = channel.attr('adiv_info');
							if(adiv.NUM!=null){
								adiv_info = adiv_info.replace(']',',{"adiv_id":"'+adiv.ADIV_ID+'","adiv_name":"'+adiv.ADIV_NAME+'('+adiv.NUM+')"}]');
							}else{
								adiv_info = adiv_info.replace(']',',{"adiv_id":"'+adiv.ADIV_ID+'","adiv_name":"'+adiv.ADIV_NAME+'"}]');
							}
							channel.attr('adiv_info', adiv_info);
						}else{
							if(adiv.NUM!=null){
								channel.attr('adiv_info', '[{"adiv_id":"'+adiv.ADIV_ID+'","adiv_name":"'+adiv.ADIV_NAME+'('+adiv.NUM+')"}]');
							}else{
								channel.attr('adiv_info', '[{"adiv_id":"'+adiv.ADIV_ID+'","adiv_name":"'+adiv.ADIV_NAME+'"}]');
							}
							
						}
					}

					for(var i = 0; i<$('#'+_lct+' .J_campType').length; i++){
						if($($('#'+_lct+' .J_campType')[i]).attr('adiv-count')>1){
							$($('#'+_lct+' .J_campType')[i]).find('span').removeClass('hidden');
						}
					}
...
function on_top_manual(channel_id,adiv_id) {
	var _url = _ctx + '/mpm/priorityAction?cmd=initManualPriorityCampseg'; 
//	var params = new Array();
	$.ajax({
		url : _url,
		type : "POST",
		data : {channelId : channel_id,adivId:adiv_id},
		success : function(result) {
			var rst = result.data;
			var html = new EJS({ url : _ctx + '/mcd/pages/EJS/effectAppraisal/sale_order_on_top.ejs' }).render({data : rst });
//			$(html).appendTo($(".content-main"));
			$("#manualList").html(html);
			//点击取消置顶按钮
			$(".stick_on_top_cancel").bind("click",function(){
				...
				$.ajax({
					type:"post",
					url:_ctx +"/mpm/priorityAction?cmd=cancleTopManualPriorityCampseg",
					data:{campsegId:campsegId,cityId:cityId,channelId:channelId,chnAdivId:chnAdivId},
			...
			$("#createDimPlanListUl").sortable({
				...
				stop:function(event, ui){
					...
					for(var i = 0,len = lis.length;i<len;i++){ 
						var priOrderNum = $(lis[i]).attr("priOrderNum");
						var campsegId = $(lis[i]).attr("campsegId");
						var cityId = $(lis[i]).attr("cityId");
						var channelId = $(lis[i]).attr("channelId");
						var chnAdivId = $(lis[i]).attr("chnAdivId");
						if(chnAdivId == undefined){
							priOrderNumStr += '{"priOrderNum":"'+priOrderNum+'","campsegId":"'+campsegId+'","cityId":"'+cityId+'","channelId":"'+channelId+'"},';
						}else{
							priOrderNumStr += '{"priOrderNum":"'+priOrderNum+'","campsegId":"'+campsegId+'","cityId":"'+cityId+'","channelId":"'+channelId+'","chnAdivId":"'+chnAdivId+'"},';
						}
					}
					priOrderNumStr = priOrderNumStr.substring(0,priOrderNumStr.length-1) + "]";
					$.ajax({
						url : _ctx + "/mpm/priorityAction?cmd=editManualPriorityCampseg",
						type : "POST",
						dataType:"json",
						data :{priOrderNumStr:priOrderNumStr  } ,
						success : function(result) {
							if(result.status == "201"){
								alert("排序错误！");
							}else{
								reloadManualList(_url,channel_id,adiv_id);
							}
						}
					});
				}
...
function reloadManualList(url,channelId,adiv_id){
	$.ajax({
		url : url,
		type : "POST",
		data : {channelId : channelId,adivId:adiv_id},
		success : function(result) {
			var rst = result.data; 
			var html = new EJS({ url : _ctx + '/mcd/pages/EJS/effectAppraisal/sale_order_on_top.ejs' }).render({data : rst });
			$("#manualList").html(html);
					...
					$.ajax({
						...
						success : function(result) {
							if(result.status == "201"){
								...
								reloadManualList(url,channelId,chnAdivId);
							}
						}
					});
				}
...
function on_top_auto(channel_id, adiv_id, page_no, is_replace, call_back) {
	...
	var _url = _ctx + '/mpm/priorityAction?cmd=initAutoPriorityCampseg';
	$.ajax({
		url : _url,
		type : "POST",
		data :{
		...
		success : function(result) {
			...
			var html = new EJS({
				url : _ctx
						+ '/mcd/pages/EJS/effectAppraisal/sale_order_auto.ejs'
			}).render({
				data : rst
			});
			if (is_replace) {
			...
			function() {
				...
				if($('.J_campType.active').find('span').hasClass('hidden')){
					...
					adiv_id = $('.adiv_tr td.active').attr('adiv_id');
						...
						adiv_id=4;
//						var ele = $('#content_offline').find('tr:first-child .J_campType:first-child');
//						add_event_listener();
//						var adiv_info = ele.attr('adiv_info');
//						adiv_info = eval("("+adiv_info+")");
//						var adiv_id = adiv_info.length>0 ? adiv_info[0].adiv_id : "";
...
//				console.debug($.trim($(this).text()));
				on_top_auto(
						$('.J_campType.active').attr('cnid'),adiv_id, $
...
//							var adiv_id = adiv_info.length>0 ? adiv_info[0].adiv_id : "";
				...
				if ('prev' == dir) {
					on_top_auto($('.J_campType.active').attr(
							'cnid'),adiv_id, Number($.trim($('a.page-num.fleft.active')
							.text())) - 1, true, 'switch_active');
				} else if ('next' == dir) {
					on_top_auto($('.J_campType.active').attr(
							'cnid'),adiv_id, Number($.trim($('a.page-num.fleft.active')
							.text())) + 1, true, 'switch_active');
					...
					on_top_auto($('.J_campType.active').attr(
					'cnid'),adiv_id, Number($.trim($('#goPageNumInput')
...
//	var search_text = document.getElementById("search_sale_order_auto").value;
//	var chanlID = $("td.fleft.content-type-box.J_campType.active").attr("cnid");
//	var pageNum = $("a.pag-num.fleft.active").text();
	...
	on_top_auto($('.J_campType.active').attr('cnid'), adiv_id,"", true, 'switch_active');
