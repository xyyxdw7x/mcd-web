!function(e) {
	...
	e.fn.onepage_scroll = function(n) {
		...
		function u(e, t) {
			...
			var _top_num = $(".left-nav-active i").attr("data-num");
			var scrollY = $(window).scrollTop();//window.scrollY;
//				if(){
//					scrollY = document.documentElement.scrollTop;
//				}
			if(t>0 && (!_top_num || _top_num == 1)){//up
				$(window).scrollTop(scrollY- t);
				return;
			}else if(t<0 && scrollY<180){
		...
		total = s.length, status = "off", topPos = 0, leftPos = 0,
				lastAnimation = 0, quietPeriod = 500, paginationList = "";
		e.fn.transformPage = function(t, n, r) {
			...
			if( n == 0){
				...
				n = -($(".box-container .box-content:nth-child(1)").height()-10);
			}
			else if(n == -200){
				n = -($(".box-container .box-content:nth-child(1)").height() + $(" .box-container .box-content:nth-child(2)").height()-10);
			}
			e(this)
					.css(
							{
								"-webkit-transform" : t.direction == "horizontal" ? "translate3d("
										+ n + "px, 0, 0)"
										: "translate3d(0, " + n + "px, 0)",
								"-webkit-transition" : "all " + t.animationTime
										+ "ms " + t.easing,
								"-moz-transform" : t.direction == "horizontal" ? "translate3d("
										+ n + "px, 0, 0)"
										: "translate3d(0, " + n + "px, 0)",
								"-moz-transition" : "all " + t.animationTime
										+ "ms " + t.easing,
								"-ms-transform" : t.direction == "horizontal" ? "translate3d("
										+ n + "px, 0, 0)"
										: "translate3d(0, " + n + "px, 0)",
								"-ms-transition" : "all " + t.animationTime
										+ "ms " + t.easing,
								transform : t.direction == "horizontal" ? "translate3d("
										+ n + "px, 0, 0)"
										: "translate3d(0, " + n + "px, 0)",
		...
		e.fn.moveDown = function() {
			...
			if (r.pagination == true) {
				e(".onepage-pagination li a" + "[data-index='" + index + "']")
						.removeClass("active");
				e(
						".onepage-pagination li a" + "[data-index='"
								+ next.data("index") + "']").addClass("active")
			}
			e(".box-container")[0].className = e(".box-container")[0].className.replace(
					/\bviewing-page-\d.*?\b/g, "");
			e(".box-container").addClass("viewing-page-" + next.data("index"));
			if (history.replaceState && r.updateURL == true) {
				var n = window.location.href.substr(0, window.location.href
						.indexOf("#"))
						+ "#" + (index + 1);
		...
		e.fn.moveTo = function(t) {
			...
			if (next.length > 0) {
				...
				e(".onepage-pagination li a" + "[data-index='" + t + "']")
						.addClass("active");
				e(".box-container")[0].className = e(".box-container")[0].className.replace(
						/\bviewing-page-\d.*?\b/g, "");
				e(".box-container").addClass("viewing-page-" + next.data("index"));
			...
			e(this).css(
					{
						...
						top : r.direction == "vertical"
								|| r.direction != "horizontal" ? topPos + "%"
								: 0
					});
		...
		if (window.location.hash != "" && window.location.hash != "#1") {
			...
			if (parseInt(init_index) <= total && parseInt(init_index) > 0) {
				e(r.sectionContainer + "[data-index='" + init_index + "']")
						.addClass("active");
				e(".box-container").addClass("viewing-page-" + init_index);
				if (r.pagination == true)
					e(
							".onepage-pagination li a" + "[data-index='"
				...
				if (next) {
					...
					e(".box-container").addClass("viewing-page-" + next.data("index"));
					if (history.replaceState && r.updateURL == true) {
						var a = window.location.href.substr(0,
				...
				e(".box-container").addClass("viewing-page-1");
				if (r.pagination == true)
					e(".onepage-pagination li a" + "[data-index='1']")
							.addClass("active")
			}
		} else {
			e(r.sectionContainer + "[data-index='1']").addClass("active");
		...
		e(document).bind(
				...
				function(t) {
					t.preventDefault();
					var n = t.originalEvent.wheelDelta
